...
function PlayingVideos() {
        ...
        async function loadSubscriptionState() {
            ...
            try {
                ...
                if (isMounted) {
                    ...
                    setSubscribedChannels(channels.map((channel) => channel._id).filter(Boolean));
                }
            } catch (error) {
                if (isMounted) {
                    setIsSubscribed(getSubscribedChannels().includes(channelId));
                }
            }
        }
    ...
    const relatedVideos = useMemo(() => {
        return videos.filter((video) => video._id !== currentVideo?._id).slice(0, 6);
    ...
    async function handleSubscribeToggle() {
        ...
        if (!channelId || !authState.isLoggedIn) {
            return;
        }

        try {
            const response = await axiosInstance.post(`subscriptions/c/${channelId}`);
            const nextSubscribed = response.data?.data?.subscribed || false;
            setIsSubscribed(nextSubscribed);
            setSubscribersCount(response.data?.data?.subscribersCount || 0);
        } catch (error) {
            const next = toggleSubscribedChannel(channelId);
    ...
    function handleLoadedMetadata() {
        if (videoRef.current) {
            setDuration(videoRef.current.duration || currentVideo?.duration || 0);
        }
    }
    ...
    if (!currentVideo) {
        return <div className="p-8 text-slate-600">Video not found.</div>;
    }

    const ownerName =
        currentVideo.owner?.fullName || currentVideo.owner?.username || "Channel";
    const ownerHandle = currentVideo.owner?.username
            ...
            <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_360px]">
                <div className="min-w-0 space-y-5">
                    ...
                    >
                        ...
                        <div
                            className={`pointer-events-none absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/30 transition-opacity duration-300 ${
                        ...
                        <button
                            type="button"
                            onClick={togglePlayPause}
                            className={`absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full bg-black/55 p-4 text-white transition ${
                                showControls ? "opacity-100" : "opacity-0"
                            }`}
                            aria-label={isPlaying ? "Pause video" : "Play video"}
                        >
                            {isPlaying ? (
                                <AiOutlinePause className="text-3xl" />
                            ) : (
                        ...
                        <div
                            className={`absolute inset-x-0 bottom-0 z-10 p-3 text-white transition-opacity duration-300 sm:p-4 ${
                                showControls ? "opacity-100" : "opacity-0"
                            }`}
                        >
                            <input
                                ...
                                value={currentTime}
                                onChange={handleSeek}
                                className="mb-3 h-1.5 w-full cursor-pointer appearance-none rounded-full bg-white/30 accent-red-600"
                            />
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                    ...
                                    <div className="flex items-center gap-2 rounded-full bg-black/25 px-3 py-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-20 cursor-pointer appearance-none rounded-full accent-white"
                                        />
                                    </div>
                                ...
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                    <select
                                        ...
                                        className="rounded-full bg-black/25 px-3 py-2 text-xs text-white outline-none sm:text-sm"
                                    >
                    ...
                    <div className="rounded-[1.75rem] bg-white p-5 shadow-sm sm:p-6">
                        ...
                        <div className="mt-3 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                            ...
                            <div className="flex flex-wrap gap-3">
                                <button
                                    ...
                                    } ${!authState.isLoggedIn ? "cursor-not-allowed opacity-60" : ""}`}
                                >
                                    <AiOutlineLike />
                                    {likesCount} Like
                                </button>
                        ...
                        <div className="mt-6 flex flex-col gap-4 rounded-3xl bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-center gap-4">
                                <img
                                    src={currentVideo.owner?.avatar || currentVideo.thumbnail}
                                    alt={ownerName}
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500">
                                        {ownerHandle}
                                        {subscribersCount ? ` • ${subscribersCount} subscribers` : ""}
                                    </p>
                                </div>
                            </div>
                            <div className="flex flex-wrap gap-3">
                                ...
                                <Link
                                    to={authState.isLoggedIn ? "/your-channel" : "/login"}
                                    className="inline-flex justify-center rounded-full border border-slate-300 px-5 py-2 text-sm font-semibold text-slate-700"
                                >
                                    Visit channel
                                </Link>
                            </div>
                ...
                <div className="space-y-4">
                    <div className="rounded-3xl bg-white p-5 shadow-sm">
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            More videos that match this channel and topic.
                        </p>
                    </div>
